...
class TestReviewService:
    ...
    def test_create_review_invalid_rating(self, review_service):
        ...
        with pytest.raises(ValueError, match="Rating must be between 1 and 5"):
            review_service.create_review(
                recipe_id="recipe1", user_id="user1", rating=6, comment="Test"
            )

        with pytest.raises(ValueError, match="Rating must be between 1 and 5"):
            review_service.create_review(
                recipe_id="recipe1", user_id="user1", rating=0, comment="Test"
            )

    def test_create_duplicate_review(self, review_service):
        """重複レビュー作成のテスト"""
        review_service.create_review(
            recipe_id="recipe1", user_id="user1", rating=5, comment="First review"
        )

        with pytest.raises(ValueError, match="already reviewed"):
            review_service.create_review(
                recipe_id="recipe1", user_id="user1", rating=4, comment="Second review"
            )
    ...
    def test_update_review(self, review_service):
        ...
        updated = review_service.update_review(
            review_id=review.id, user_id="user1", rating=4, comment="Good but not great"
        )
    ...
    def test_update_review_unauthorized(self, review_service):
        ...
        with pytest.raises(ValueError, match="Permission denied"):
            review_service.update_review(review_id=review.id, user_id="user2", rating=4)
    ...
    def test_update_review_not_found(self, review_service):
        ...
        with pytest.raises(ValueError, match="Review not found"):
            review_service.update_review(
                review_id="nonexistent", user_id="user1", rating=4
            )
    ...
    def test_comment_max_length(self, review_service):
        ...
        long_comment = "a" * 3000
        review = review_service.create_review(
            recipe_id="recipe1", user_id="user1", rating=5, comment=long_comment
        )
