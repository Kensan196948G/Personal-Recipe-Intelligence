...
class ProductSearchRequest(BaseModel):
    ...
    services: Optional[List[DeliveryServiceType]] = Field(
        None, description="検索対象サービス（未指定時は全サービス）"
    )
...
async def get_services():
    ...
    try:
        ...
        return {"status": "ok", "data": {"services": services, "count": len(services)}}
...
async def search_products(request: ProductSearchRequest):
    ...
    try:
        ...
        return {
            "status": "ok",
            "data": {
                "products": [delivery_service._product_to_dict(p) for p in products],
                "count": len(products),
                "query": {
                    "ingredient_name": request.ingredient_name,
                    "services": request.services,
...
@router.get("/cart")
async def get_cart(service: DeliveryServiceType = Query(..., description="サービス")):
    """
    カート内容取得
...
async def remove_from_cart(request: RemoveFromCartRequest):
    ...
    try:
        result = delivery_service.remove_from_cart(
            service=request.service, product_id=request.product_id
        )
        return {"status": "ok", "data": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def clear_cart(service: DeliveryServiceType = Query(..., description="サービス")):
    """
    カートクリア
...
async def compare_prices(ingredient_name: str = Query(..., description="食材名")):
    """
    価格比較
