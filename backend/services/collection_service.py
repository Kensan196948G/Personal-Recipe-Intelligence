...
class CollectionService:
    ...
    def __init__(
        self, data_dir: str = "/mnt/Linux-ExHDD/Personal-Recipe-Intelligence/data"
    ):
        """
    ...
    def _save_collections(self, collections: List[Collection]) -> None:
        ...
        with open(self.collections_file, "w", encoding="utf-8") as f:
            json.dump(
                [c.to_dict() for c in collections], f, ensure_ascii=False, indent=2
            )
    ...
    def copy_collection(
        ...
        if (
            source.visibility != CollectionVisibility.PUBLIC
            and source.owner_id != user_id
        ):
            return None
