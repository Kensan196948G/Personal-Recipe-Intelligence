...
class RecipeGeneratorService:
    ...
    def generate_recipe(
        ...
        # 材料リストを作成
        all_ingredients = (
            [main_ingredient] + sub_ingredients + template["base_ingredients"]
        )
    ...
    def generate_variations(
        self, base_recipe: Dict[str, Any], count: int = 3
    ) -> List[Dict[str, Any]]:
        """
        既存レシピのバリエーションを生成
        ...
        main_ingredient = (
            base_recipe["ingredients"][0]["name"]
        ...
        for i in range(min(count, len(available_methods))):
            ...
            # 食材リストを取得
            ingredients = [ing["name"] for ing in base_recipe.get("ingredients", [])][
                :3
            ]
    ...
    def suggest_ingredient_combinations(
        self, main_ingredient: str, count: int = 5
    ) -> List[Dict[str, Any]]:
        """
        食材の組み合わせを提案
    ...
    def improve_recipe(
        self, recipe: Dict[str, Any], focus: str = "taste"
    ) -> Dict[str, Any]:
        """
        既存レシピを改善
        ...
        if focus == "taste":
            ...
            for seasoning in seasonings[:2]:
                if not any(
                    ing["name"] == seasoning for ing in improved.get("ingredients", [])
                ):
                    improved.setdefault("ingredients", []).append(
                        {"name": seasoning, "amount": "少々", "unit": ""}
                    )
            ...
            improved["steps"] = [
                step.replace("油で揚げる", "オーブンで焼く").replace(
                    "多めの油", "少量の油"
                )
                for step in steps
            ]
    ...
    def get_nutrition_estimate(self, recipe: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": (
                "バランスが良い" if balance_score > 66 else "野菜を追加するとより良い"
            ),
        }
