...
class AdminService:
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        active_users = sum(
            1 for u in users if u.get("last_login_at", "") >= thirty_days_ago
        )

        # 最近の活動
        week_ago = (now - timedelta(days=7)).isoformat()
        recent_recipes = sum(1 for r in recipes if r.get("created_at", "") >= week_ago)
    ...
    def get_recipe_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        # 期間内のレシピをフィルタ
        period_recipes = [r for r in recipes if r.get("created_at", "") >= start_date]
        ...
        # 平均人数
        servings = [
            r.get("servings", 0) for r in period_recipes if r.get("servings", 0) > 0
        ]
    ...
    def get_user_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        # 新規ユーザー
        new_users = sum(1 for u in users if u.get("created_at", "") >= start_date)

        # アクティブユーザー（レシピ作成数でランキング）
        user_recipe_counts = []
        for user in users:
            user_id = user.get("id")
            recipe_count = sum(1 for r in recipes if r.get("user_id") == user_id)
            if recipe_count > 0:
