...
class ReportService:
  ...
  def _generate_report(
    ...
    nutrition_summary = NutritionSummary(
      ...
      avg_daily_calories=nutrition_data.get("total_calories", 0) / days if days > 0 else 0,
      avg_daily_protein=nutrition_data.get("total_protein", 0) / days if days > 0 else 0,
      avg_daily_fat=nutrition_data.get("total_fat", 0) / days if days > 0 else 0,
      avg_daily_carbs=nutrition_data.get("total_carbs", 0) / days if days > 0 else 0
    )
  ...
  def _generate_recommendations(
    ...
    if nutrition.avg_daily_calories < 1800:
      recommendations.append("カロリー摂取量が少ない傾向にあります。バランスの良い食事を心がけましょう。")
    elif nutrition.avg_daily_calories > 2500:
      recommendations.append("カロリー摂取量が多めです。野菜中心の食事を増やすことをお勧めします。")

    if nutrition.avg_daily_protein < 50:
      recommendations.append("タンパク質が不足しています。魚・肉・豆類を積極的に摂取しましょう。")

    # 支出チェック
    if expense.avg_expense_per_meal > 1000:
      recommendations.append("1食あたりの支出が高めです。自炊を増やすことでコスト削減が可能です。")

    # 目標達成率チェック
    if goal.completion_rate < 50 and goal.total_goals > 0:
      recommendations.append("目標達成率が低めです。小さな目標から始めて達成感を積み重ねましょう。")
    elif goal.completion_rate >= 80:
      recommendations.append("素晴らしい達成率です！この調子で継続しましょう。")

    if not recommendations:
      recommendations.append("バランスの取れた食生活を継続できています。この調子で頑張りましょう！")
  ...
  def generate_pdf(
    ...
    # 期間情報
    story.append(Paragraph(f"期間: {report_data.start_date} 〜 {report_data.end_date}", normal_style))
    ...
    nutrition_data = [
      ["項目", "合計", "1日平均"],
      ["カロリー", f"{report_data.nutrition_summary.total_calories:.1f} kcal",
       f"{report_data.nutrition_summary.avg_daily_calories:.1f} kcal"],
      ["タンパク質", f"{report_data.nutrition_summary.total_protein:.1f} g",
       f"{report_data.nutrition_summary.avg_daily_protein:.1f} g"],
      ["脂質", f"{report_data.nutrition_summary.total_fat:.1f} g",
       f"{report_data.nutrition_summary.avg_daily_fat:.1f} g"],
      ["炭水化物", f"{report_data.nutrition_summary.total_carbs:.1f} g",
    ...
    nutrition_table.setStyle(TableStyle([
      ...
      ('FONTNAME', (0, 0), (-1, -1), 'Japanese' if 'Japanese' in pdfmetrics.getRegisteredFontNames() else 'Helvetica'),
      ('FONTSIZE', (0, 0), (-1, -1), 10),
