...
class TestRecipeGeneratorService:
  ...
  def test_suggest_ingredient_combinations_unknown_ingredient(self, generator_service):
    """未知の食材での組み合わせ提案のテスト"""
    suggestions = generator_service.suggest_ingredient_combinations(
      main_ingredient="未知の食材",
      count=5
    )
  ...
  def test_recipe_structure(self, generator_service):
    ...
    # 必須フィールドの確認
    required_fields = [
      "id", "name", "category", "cooking_time", "difficulty",
      "ingredients", "steps", "servings", "tags", "generated_at"
    ]
