...
class TranslationCache:
  ...
  def _generate_key(self, text: str, target_lang: str, source_lang: Optional[str] = None) -> str:
    """
  ...
  def set(self, text: str, target_lang: str, translation: str, source_lang: Optional[str] = None) -> None:
    """
  ...
  def translate_batch(
    ...
    if uncached_texts:
      try:
        ...
        logger.info(f"Batch translated {len(uncached_texts)} texts to {target_lang.value}")
    ...
    try:
      ...
      return {
        ...
        "character_usage_percent": (
          (usage.character.count / usage.character.limit * 100) if usage.character.limit else 0
        ),
      }
    except deepl.DeepLException as e:
