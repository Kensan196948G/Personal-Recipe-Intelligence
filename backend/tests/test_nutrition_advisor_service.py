...
class TestNutritionAdvisorService:
    ...
    def test_get_chat_history_pagination(self, advisor_service, sample_user_id):
        ...
        result = advisor_service.get_chat_history(
            user_id=sample_user_id, limit=4, offset=0
        )
    ...
    def test_analyze_meal_basic(self, advisor_service, sample_user_id):
        ...
        result = advisor_service.analyze_meal(
            user_id=sample_user_id, meal_data=meal_data
        )
    ...
    def test_generate_meal_plan_default(self, advisor_service, sample_user_id):
        ...
        result = advisor_service.generate_meal_plan(
            user_id=sample_user_id, preferences={}
        )
    ...
    def test_generate_meal_plan_muscle_gain_goal(self, advisor_service, sample_user_id):
        ...
        assert any(
            "筋肉" in advice or "タンパク質" in advice
            for advice in result["general_advice"]
        )
    ...
    def test_generate_meal_plan_with_restrictions(
        ...
        assert any(
            "血糖値" in advice or "糖尿病" in advice
            for advice in result["general_advice"]
        )
    ...
    def test_personalized_response(self, advisor_service, sample_user_id):
        ...
        # 減量目標が反映されているか確認
        assert (
            "減量" in result["message"]["content"]
            or "カロリー" in result["message"]["content"]
        )
    ...
    def test_user_profile_persistence(
        self, advisor_service, sample_user_id, test_data_dir
    ):
        """ユーザープロファイルの永続化テスト"""
        # プロファイル更新
        advisor_service.update_user_profile(
