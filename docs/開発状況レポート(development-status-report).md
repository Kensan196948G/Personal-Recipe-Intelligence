...
### 3.3 v0.2.0 修正内容（2025-12-12）

1. **WebUI動作修正**
   - collector routerをapp.pyに登録（海外レシピ収集機能有効化）
   - データベース自動初期化機能追加
   - APIレスポンス形式の統一（スタブ→本実装ルーター切替）

2. **レート制限の実装** (`backend/middleware/rate_limiter.py`)
   - slowapiライブラリ導入
   - OCR: 5 requests/minute
   - Video: 5 requests/minute
   - Scraper: 10 requests/minute
...
1. ~~修正済みのセキュリティ改善をコミット~~ ✅ 完了
2. CIでテストが通過することを確認

### 短期（1週間以内）

1. ~~レート制限の実装~~ ✅ 完了（slowapi導入済み）
2. 主要サービスのテスト追加
...
## 10. v0.2.0 リリースノート（2025-12-12）

### WebUI動作確認済み機能

| 機能 | ステータス | 備考 |
|------|----------|------|
| レシピ一覧表示 | ✅ 動作 | 10件/ページ、新しい順 |
| レシピ詳細表示 | ✅ 動作 | |
| レシピ作成 | ✅ 動作 | |
| レシピ編集 | ✅ 動作 | |
| レシピ削除 | ✅ 動作 | |
| CSVインポート | ✅ 動作 | |
| 海外レシピ収集 | ✅ 動作 | Spoonacular + DeepL |
| 検索機能 | ✅ 動作 | |
| ページネーション | ✅ 動作 | |

### アクセス情報

- **バックエンドAPI**: http://192.168.0.187:8001
- **フロントエンドUI**: http://192.168.0.187:5173
- **API Docs**: http://192.168.0.187:8001/api/docs
...
**最終更新:** 2025-12-12 by Claude Code
