...
class VoiceAssistantService:
    ...
    def process_google_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if intent_name == "actions.intent.MAIN":
                ...
                return self._process_intent(
                    session_id, intent_name, params, VoiceAssistant.GOOGLE
                )
    ...
    def process_generic_command(self, session_id: str, command: str) -> Dict[str, Any]:
        """汎用音声コマンド処理"""
        try:
            # 簡易的な意図解析
            ...
            if "レシピ" in command_lower or "recipe" in command_lower:
                # レシピ検索
                query = (
                    command_lower.replace("レシピ", "").replace("recipe", "").strip()
                )
                return self._process_intent(
                    session_id,
                ...
                return self._process_intent(
                    session_id, IntentType.NEXT_STEP, {}, VoiceAssistant.GENERIC
                )
            elif "前" in command_lower or "previous" in command_lower:
                return self._process_intent(
                    session_id, IntentType.PREVIOUS_STEP, {}, VoiceAssistant.GENERIC
                )
            elif "もう一度" in command_lower or "repeat" in command_lower:
                return self._process_intent(
                    session_id, IntentType.REPEAT_STEP, {}, VoiceAssistant.GENERIC
                )
            elif "材料" in command_lower or "ingredients" in command_lower:
                return self._process_intent(
                    session_id, IntentType.GET_INGREDIENTS, {}, VoiceAssistant.GENERIC
                )
                ...
                return self._process_intent(
                    session_id, IntentType.HELP, {}, VoiceAssistant.GENERIC
                )
            else:
                return {
                    "status": "error",
    ...
    def _process_intent(
        ...
        if intent_name == IntentType.SEARCH_RECIPE:
            ...
            return self._format_response(
                "そのコマンドには対応していません", assistant, should_end_session=False
            )
    ...
    def _handle_next_step(
        ...
        if not steps:
            ...
            self._update_session(
                session["session_id"], {"current_step_index": next_index}
            )
    ...
    def _handle_previous_step(
        ...
        if not steps:
            ...
            self._update_session(
                session["session_id"], {"current_step_index": prev_index}
            )
    ...
    def _handle_cancel(
        ...
        self._update_session(
            session["session_id"],
            {"current_recipe_id": None, "current_step_index": 0, "context": {}},
        )
    ...
    def _format_response(
        ...
        if assistant == VoiceAssistant.ALEXA:
            return self._alexa_response(
                speech_text, should_end_session, card_title, card_content
            )
    ...
    def _google_response(
        self, speech_text: str, should_end_session: bool = False
    ) -> Dict[str, Any]:
        ...
        response = {
            ...
            "prompt": {
                "override": False,
                "content": {"speech": f"<speak>{speech_text}</speak>"},
            },
        }

        if should_end_session:
            response["scene"] = {"name": "actions.scene.END_CONVERSATION"}
