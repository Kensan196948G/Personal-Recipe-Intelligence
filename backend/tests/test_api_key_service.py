...
class TestAPIKeyGeneration:
    ...
    def test_generate_api_key_custom_scope(self, service):
        ...
        raw_key, api_key = service.generate_api_key(name="Full Access Key", scope=scope)
    ...
    def test_generate_api_key_custom_rate_limit(self, service):
        ...
        rate_limit = RateLimit(
            requests_per_minute=120, requests_per_hour=5000, requests_per_day=50000
        )

        raw_key, api_key = service.generate_api_key(
            name="High Limit Key", rate_limit=rate_limit
        )
...
class TestRateLimiting:
    ...
    def test_rate_limit_cleanup_old_records(self, service):
        ...
        rate_limit = RateLimit(requests_per_minute=5)
        raw_key, api_key = service.generate_api_key(
            name="Test Key", rate_limit=rate_limit
        )

        # 古いタイムスタンプで記録を追加
        old_time = time.time() - 90000  # 25時間前
        for i in range(5):
            record = UsageRecord(timestamp=old_time, endpoint="/test", status_code=200)
