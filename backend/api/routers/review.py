...
class UpdateReviewRequest(BaseModel):
    ...
    comment: Optional[str] = Field(
        None, min_length=1, max_length=2000, description="コメント"
    )
...
async def get_recipe_reviews(
    ...
    try:
        ...
        for review in reviews:
            is_helpful = (
                current_user_id in review.helpful_users if current_user_id else False
            )
            review_list.append(
                {
                    "id": review.id,
        ...
        return ApiResponse(
            status="ok", data={"reviews": review_list, "summary": summary.to_dict()}
        )
    except Exception as e:
...
async def get_user_reviews(
    ...
    try:
        ...
        return ApiResponse(status="ok", data={"reviews": review_list})
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get user reviews: {str(e)}"
        )
...
async def update_review(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update review: {str(e)}"
        )


@router.delete("/{review_id}")
async def delete_review(
    review_id: str, authorization: Optional[str] = Header(None)
) -> ApiResponse:
    """
    レビューを削除
    ...
    try:
        ...
        return ApiResponse(status="ok", data={"message": "Review deleted successfully"})
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except HTTPException:
        raise
...
async def get_rating_summary(recipe_id: str) -> ApiResponse:
    ...
    try:
        ...
        return ApiResponse(status="ok", data={"summary": summary.to_dict()})
    except Exception as e:
        raise HTTPException(
...
async def get_popular_reviews(recipe_id: str, limit: int = 3) -> ApiResponse:
    """
    人気レビューを取得
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get popular reviews: {str(e)}"
        )
