...
class TestAPIKeyAuthentication:
    ...
    def test_request_with_invalid_api_key(self, client):
        """無効なAPIキーでのリクエスト"""
        response = client.get("/api/v1/recipes", headers={"X-API-Key": "invalid_key"})
    ...
    def test_request_with_valid_api_key_header(self, client, api_key_service):
        ...
        response = client.get("/api/v1/recipes", headers={"X-API-Key": raw_key})
    ...
    def test_request_with_valid_api_key_bearer(self, client, api_key_service):
        ...
        response = client.get(
            "/api/v1/recipes", headers={"Authorization": f"Bearer {raw_key}"}
        )
...
class TestRateLimiting:
    ...
    def test_rate_limit_exceeded(self, client, api_key_service):
        ...
        raw_key, api_key = api_key_service.generate_api_key(
            name="Limited Key", rate_limit=rate_limit
        )

        # 2回は成功
        response1 = client.get("/api/v1/recipes", headers={"X-API-Key": raw_key})
        assert response1.status_code == 200

        response2 = client.get("/api/v1/recipes", headers={"X-API-Key": raw_key})
        assert response2.status_code == 200

        # 3回目は制限
        response3 = client.get("/api/v1/recipes", headers={"X-API-Key": raw_key})
...
class TestScopeValidation:
    ...
    def test_read_recipes_with_read_scope(self, client, api_key_service):
        ...
        raw_key, api_key = api_key_service.generate_api_key(
            name="Read Only Key", scope=scope
        )
    ...
    def test_write_recipes_without_write_scope(self, client, api_key_service):
        ...
        response = client.post("/api/v1/recipes", headers={"X-API-Key": raw_key})
    ...
    def test_delete_recipes_without_delete_scope(self, client, api_key_service):
        """削除スコープなしでレシピ削除"""
        scope = APIKeyScope(read_recipes=True, write_recipes=True, delete_recipes=False)
        raw_key, api_key = api_key_service.generate_api_key(
            name="No Delete Key", scope=scope
        )

        response = client.delete("/api/v1/recipes/123", headers={"X-API-Key": raw_key})
    ...
    def test_delete_recipes_with_delete_scope(self, client, api_key_service):
        """削除スコープでレシピ削除"""
        scope = APIKeyScope(read_recipes=True, write_recipes=True, delete_recipes=True)
        raw_key, api_key = api_key_service.generate_api_key(
            name="Full Access Key", scope=scope
        )
    ...
    def test_read_tags_with_read_scope(self, client, api_key_service):
        ...
        raw_key, api_key = api_key_service.generate_api_key(
            name="Tag Read Key", scope=scope
        )

        response = client.get("/api/v1/tags", headers={"X-API-Key": raw_key})
    ...
    def test_write_tags_without_write_scope(self, client, api_key_service):
        ...
        scope = APIKeyScope(read_tags=True, write_tags=False)
        raw_key, api_key = api_key_service.generate_api_key(
            name="Tag Read Only Key", scope=scope
        )

        response = client.post("/api/v1/tags", headers={"X-API-Key": raw_key})
