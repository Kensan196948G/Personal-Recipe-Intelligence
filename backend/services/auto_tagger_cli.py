...
def batch_mode(tagger: AutoTagger, json_file: str):
    ...
    try:
        ...
        for i, recipe in enumerate(recipes, 1):
            ...
            results.append({"title": title, "suggested_tags": tags})
...
使用例:
  ...
  %(prog)s --list-categories
    ...
    parser.add_argument("--title", "-t", type=str, help="レシピのタイトル")
    parser.add_argument("--description", "-d", type=str, help="レシピの説明")
    parser.add_argument("--ingredients", "-i", type=str, help="材料（カンマ区切り）")
    parser.add_argument("--instructions", "-s", type=str, help="手順（カンマ区切り）")
    parser.add_argument("--max-tags", "-m", type=int, help="最大タグ数")

    # Output options
    parser.add_argument(
        "--categorized", "-c", action="store_true", help="カテゴリ別にタグを表示"
    )
    ...
    parser.add_argument(
        "--batch", "-b", type=str, metavar="FILE", help="JSONファイルからバッチ処理"
    )

    # Info options
    parser.add_argument(
        "--list-tags", action="store_true", help="利用可能なタグ一覧を表示"
    )
    parser.add_argument(
        "--list-categories", action="store_true", help="カテゴリ一覧を表示"
    )
    parser.add_argument(
        "--category", type=str, help="特定カテゴリのタグを表示（--list-tagsと併用）"
    )

    # Custom rules file
    parser.add_argument("--rules", type=str, help="カスタムルールファイルのパス")
