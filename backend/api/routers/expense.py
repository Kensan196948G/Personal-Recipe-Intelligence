...
class BudgetRequest(BaseModel):
    ...
    category_budgets: Optional[Dict[str, float]] = Field(
        None, description="カテゴリ別予算"
    )
...
async def record_expense(request: ExpenseRecordRequest):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid date format. Use ISO8601 format."
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to record expense: {str(e)}"
        )
...
async def set_budget(request: BudgetRequest):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid month format. Use YYYY-MM format."
            )
...
async def get_budget(month: str = Query(..., description="対象月（YYYY-MM形式）")):
    ...
    try:
        ...
        if budget is None:
            return ApiResponse(status="ok", data={"budget": None})
...
async def get_category_breakdown(
    ...
    try:
        ...
        return ApiResponse(status="ok", data={"breakdown": breakdown})

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get category breakdown: {str(e)}"
        )


@router.get("/trends", response_model=ApiResponse)
async def get_trends(
    months: int = Query(6, ge=1, le=24, description="過去何ヶ月分を取得するか"),
):
    """
    支出傾向を取得
    ...
    try:
        ...
        return ApiResponse(status="ok", data={"trends": trends})
    ...
    try:
        ...
        if start_date:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Invalid start_date format. Use ISO8601 format.",
                )
        ...
        if end_date:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Invalid end_date format. Use ISO8601 format.",
                )
        ...
        expenses = expense_service.get_expenses(
            start_date=start_date, end_date=end_date, category=category_enum
        )
