...
class CreateAPIKeyRequest(BaseModel):
    ...
    requests_per_minute: int = Field(
        60, ge=1, le=1000, description="分あたりリクエスト数"
    )
    requests_per_hour: int = Field(
        1000, ge=1, le=10000, description="時間あたりリクエスト数"
    )
    requests_per_day: int = Field(
        10000, ge=1, le=100000, description="日あたりリクエスト数"
    )
...
class CreateAPIKeyResponse(BaseModel):
    ...
    message: str = (
        "API key created successfully. Please save this key securely as it won't be shown again."
    )
...
async def create_api_key(
    request: CreateAPIKeyRequest, service: APIKeyService = Depends(get_api_key_service)
):
    """
    新しいAPIキーを発行
    ...
    try:
        ...
        api_key, key_info = service.generate_api_key(
            name=request.name, scope=scope, rate_limit=rate_limit
        )
...
@router.get("/keys", response_model=APIResponse, summary="APIキー一覧を取得")
async def list_api_keys(service: APIKeyService = Depends(get_api_key_service)):
    """
    登録されているすべてのAPIキーを取得
    ...
    try:
        ...
        return APIResponse(status="ok", data={"keys": keys, "total": len(keys)})
...
@router.get(
    "/keys/{key_id}", response_model=APIResponse, summary="特定のAPIキー情報を取得"
)
async def get_api_key(
    ...
    return APIResponse(status="ok", data=key_info)


@router.delete("/keys/{key_id}", response_model=APIResponse, summary="APIキーを削除")
async def delete_api_key(
    ...
    return APIResponse(
        status="ok", data={"message": "API key deleted successfully", "key_id": key_id}
    )


@router.patch(
    "/keys/{key_id}/revoke", response_model=APIResponse, summary="APIキーを無効化"
)
async def revoke_api_key(
    ...
    return APIResponse(
        status="ok", data={"message": "API key revoked successfully", "key_id": key_id}
    )
...
@router.get("/usage", response_model=APIResponse, summary="全体の使用量統計を取得")
async def get_overall_usage(service: APIKeyService = Depends(get_api_key_service)):
    """
    すべてのAPIキーの使用量統計を取得
    """
    try:
        ...
        return APIResponse(
            status="ok", data={"usage_stats": all_stats, "total_keys": len(all_stats)}
        )
...
@router.get(
    "/usage/{key_id}", response_model=APIResponse, summary="特定のAPIキーの使用量を取得"
)
...
@router.get("/docs", response_model=APIResponse, summary="API仕様ドキュメントを取得")
async def get_api_docs():
    ...
    docs = {
        ...
        "response_format": {
            "success": {"status": "ok", "data": {}, "error": None},
            "error": {"status": "error", "data": None, "error": "error message"},
        },
        "example_usage": {
    ...
    return APIResponse(status="ok", data=docs)
