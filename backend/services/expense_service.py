...
class ExpenseRecord:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "category": (
                self.category.value
                if isinstance(self.category, ExpenseCategory)
                else self.category
            ),
...
class ExpenseService:
    ...
    def _save_expenses(self, expenses: List[ExpenseRecord]) -> None:
        ...
        try:
            with open(self.expenses_file, "w", encoding="utf-8") as f:
                json.dump(
                    [exp.to_dict() for exp in expenses], f, ensure_ascii=False, indent=2
                )
        except Exception as e:
    ...
    def get_summary(
        self, period: str = "month", date: Optional[str] = None
    ) -> ExpenseSummary:
        """
        支出サマリーを取得
        ...
        for category in ExpenseCategory:
            ...
            category_breakdown[category.value] = sum(
                e.amount for e in category_expenses
            )
    ...
    def get_trends(self, months: int = 6) -> Dict[str, List[Dict]]:
        ...
        Returns:
            月次推移データ
        """
        trends = {"monthly_totals": [], "category_trends": {}}
        ...
        for i in range(months):
            ...
            trends["monthly_totals"].insert(0, {"month": month_str, "total": total})
            ...
            for category in ExpenseCategory:
                ...
                trends["category_trends"][category.value].insert(
                    0, {"month": month_str, "total": category_total}
                )
