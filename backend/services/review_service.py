...
class ReviewService:
    ...
    def create_review(
        self, recipe_id: str, user_id: str, rating: int, comment: str
    ) -> Review:
        """
        レビューを投稿
    ...
    def get_recipe_reviews(
        self, recipe_id: str, sort_by: str = "recent", limit: Optional[int] = None
    ) -> list[Review]:
        """
        レシピのレビュー一覧を取得
        ...
        if sort_by == "rating":
            ...
            recipe_reviews.sort(
                key=lambda x: (-x.helpful_count, -x.created_at.timestamp())
            )
        else:  # recent
