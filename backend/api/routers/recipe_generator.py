...
class GenerateRecipeRequest(BaseModel):
  ...
  category: str = Field(default="japanese", description="料理カテゴリ (japanese/western/chinese)")
  cooking_time: Optional[int] = Field(default=None, ge=5, le=120, description="調理時間（分）")
...
class ImproveRecipeRequest(BaseModel):
  ...
  focus: str = Field(default="taste", description="改善の焦点 (taste/health/speed/cost)")
...
async def improve_recipe(request: ImproveRecipeRequest):
  ...
  try:
    ...
    if request.focus not in valid_focuses:
      raise ValueError(f"無効な改善焦点: {request.focus}。{valid_focuses} から選択してください。")
