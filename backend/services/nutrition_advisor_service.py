...
class NutritionAdvisorService:
  ...
  def _initialize_knowledge_base(self) -> Dict[str, Any]:
    ...
    return {
      "greetings": {
        "patterns": [r"こんにちは", r"はじめまして", r"こんばんは", r"おはよう"],
        "responses": [
      ...
      "carbohydrates": {
        "patterns": [r"炭水化物", r"糖質", r"ご飯", r"パン", r"麺", r"エネルギー"],
        "responses": [
      ...
      "minerals": {
        "patterns": [r"ミネラル", r"カルシウム", r"鉄分", r"亜鉛", r"マグネシウム"],
        "responses": [
  ...
  def _initialize_quick_actions(self) -> List[Dict[str, str]]:
    """クイックアクションを初期化"""
    return [
      {"id": "weight_loss", "label": "ダイエットのコツは？", "message": "ダイエットについて教えてください"},
      {"id": "protein", "label": "タンパク質の摂り方は？", "message": "タンパク質について教えてください"},
      {"id": "balanced_diet", "label": "バランスの良い食事とは？", "message": "バランスの良い食事について教えてください"},
      {"id": "hydration", "label": "水分補給のタイミングは？", "message": "水分補給について教えてください"},
      {"id": "meal_timing", "label": "食事のタイミングは？", "message": "食事時間について教えてください"},
      {"id": "vitamins", "label": "ビタミンの重要性は？", "message": "ビタミンについて教えてください"},
    ]
  ...
  def analyze_meal(self, user_id: str, meal_data: Dict) -> Dict[str, Any]:
    ...
    # タンパク質チェック
    if total_protein < 15:
      advice.append("タンパク質が不足しています。肉、魚、卵、大豆製品を追加しましょう。")
      score -= 10
    ...
    if carb_ratio > 70:
      advice.append("炭水化物の割合が高めです。野菜やタンパク質を増やしましょう。")
      score -= 5
  ...
  def get_daily_tip(self, user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    categories = [
      cat for cat in self.knowledge_base.keys() if cat not in ["unknown", "greetings"]
    ]
  ...
  def generate_meal_plan(
    ...
    meal_plan = {
      ...
      "meals": {
        "breakfast": {
          ...
          "tips": ["タンパク質と炭水化物をバランスよく", "果物でビタミンを補給"],
        },
        "lunch": {
          ...
          "tips": ["活動エネルギー源として炭水化物を摂取", "野菜をたっぷり取り入れる"],
        },
        "dinner": {
    ...
    if "weight_loss" in goals:
      ...
      meal_plan["general_advice"].append("運動後30分以内にタンパク質を補給しましょう")

    # 制限事項に応じたアドバイス
    restrictions = preferences.get("restrictions", user_profile.get("restrictions", []))
    if "diabetes" in restrictions:
      meal_plan["general_advice"].append(
        "血糖値の急上昇を防ぐため、低GI食品を選びましょう"
      )
    if "allergy" in restrictions:
      meal_plan["general_advice"].append("アレルゲンを含まない代替食材を選びましょう")
