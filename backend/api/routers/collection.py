...
async def get_public_collections(
    ...
    """
    try:
        collections = collection_service.get_public_collections(
...
async def get_collection(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get collection: {str(e)}"
        )
...
@router.delete(
    "/{collection_id}/recipes/{recipe_id}", response_model=CollectionResponse
)
async def remove_recipe_from_collection(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to remove recipe: {str(e)}"
        )
...
async def copy_collection(
    ...
    try:
        ...
        if not collection:
            raise HTTPException(
                status_code=404, detail="Collection not found or not accessible"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to copy collection: {str(e)}"
        )
