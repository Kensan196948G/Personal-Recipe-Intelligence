...
class DelishKitchenScraper(BaseScraper):
  ...
  async def scrape(self, url: str) -> Dict[str, Any]:
    ...
    if recipe:
      ...
      logger.info(f"Extracted recipe from Delish Kitchen using schema.org: {recipe['title']}")
      return recipe

    # Fallback to Delish Kitchen-specific parsing
    return self._parse_delish_kitchen_specific(soup, url)

  def _parse_delish_kitchen_specific(self, soup: BeautifulSoup, url: str) -> Dict[str, Any]:
    """
    ...
    if not servings_elem:
      servings_elem = soup.find(string=lambda text: text and ("人前" in text or "人分" in text))
    if servings_elem:
      import re
  ...
  def _extract_delish_kitchen_ingredients(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
    """
  ...
  def _extract_delish_kitchen_steps(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
    """
