...
class APIKeyMiddleware:
    ...
    async def __call__(self, request: Request, call_next: Callable):
        ...
        if not key_info:
            return JSONResponse(
                status_code=status.HTTP_401_UNAUTHORIZED,
                content={"status": "error", "error": "Invalid API key", "data": None},
            )

        # レート制限チェック
        is_allowed, error_msg = self.api_key_service.check_rate_limit(key_info.key_id)
        if not is_allowed:
            return JSONResponse(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                content={"status": "error", "error": error_msg, "data": None},
            )
