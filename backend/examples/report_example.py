...
def main():
  ...
  print(f"  - 1日平均カロリー: {weekly_report.nutrition_summary.avg_daily_calories:.1f} kcal")
  ...
  # PDF 生成
  print("\n[5] PDF レポート生成...")
  pdf_path = Path("data/reports") / f"example_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
  ...
  # HTML 生成
  print("\n[6] HTML レポート生成...")
  html_path = Path("data/reports") / f"example_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
  ...
  # Markdown 生成
  print("\n[7] Markdown レポート生成...")
  markdown_path = Path("data/reports") / f"example_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
