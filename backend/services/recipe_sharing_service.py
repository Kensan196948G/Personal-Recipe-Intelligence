...
class RecipeSharingService:
  ...
  def get_share_stats(self, owner_id: str) -> Dict[str, Any]:
    ...
    return {
      ...
      "view_only_shares": len([s for s in shares if s.permission == SharePermission.VIEW_ONLY]),
      "edit_shares": len([s for s in shares if s.permission == SharePermission.EDIT])
    }
