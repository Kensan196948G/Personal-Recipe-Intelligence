...
def test_generate_monthly_report(report_service):
  ...
  # 今月のレポート生成
  report_data = report_service.generate_monthly_report(user_id=user_id, month_offset=0)
...
def test_report_history_save_and_load(report_service):
  ...
  report_data2 = report_service.generate_monthly_report(user_id=user_id, month_offset=0)
...
def test_report_date_validation(report_service):
  ...
  # 週次レポート
  weekly_report = report_service.generate_weekly_report(user_id=user_id, week_offset=0)
  ...
  # 月次レポート
  monthly_report = report_service.generate_monthly_report(user_id=user_id, month_offset=0)
