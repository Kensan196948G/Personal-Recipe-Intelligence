...
class FollowService:
  ...
  def get_followers(
    ...
    follower_relations = [
      f for f in follows if f.get("following_id") == user_id
    ]
  ...
  def get_following(
    ...
    following_relations = [
      f for f in follows if f.get("follower_id") == user_id
    ]
  ...
  def get_follow_feed(self, user_id: str, limit: int = 20) -> List[Dict]:
    ...
    # フォロー中ユーザーのレシピを抽出
    following_recipes = [
      r for r in recipes if r.get("user_id") in following_ids
    ]

    # 作成日時でソート（新しい順）
    following_recipes.sort(
