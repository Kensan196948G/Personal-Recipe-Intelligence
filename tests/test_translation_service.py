...
class TestTranslationCache:
  ...
  def test_cache_expiration(self):
    ...
    cache._cache[list(cache._cache.keys())[0]]["timestamp"] = datetime.now() - timedelta(minutes=1)
    result = cache.get("Hello", "JA")
    assert result is None
...
class TestTranslationService:
  ...
  def test_translate_batch(self, mock_translator):
    ...
    service = TranslationService("test_api_key")
    results = service.translate_batch(["Hello", "Goodbye"], Language.JA, use_cache=False)
