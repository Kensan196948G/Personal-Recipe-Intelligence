...
def verify_device_auth(device_id: str, api_key: Optional[str]) -> None:
    ...
    if not iot_service.verify_api_key(device_id, api_key):
        raise HTTPException(
            status_code=403, detail="Invalid API key or inactive device"
        )
...
def get_devices(
    is_active: Optional[bool] = Query(None, description="アクティブフィルター"),
):
    """
    デバイス一覧取得
...
def sync_inventory(
    ...
    try:
        ...
        items = iot_service.sync_inventory(
            device_id=request.device_id, items=[item.dict() for item in request.items]
        )
        ...
        return APIResponse(
            status="ok", data={"synced_count": len(item_list), "items": item_list}
        )
    except HTTPException:
        raise
    except Exception as e:
...
def get_alerts(
    ...
    try:
        alerts = iot_service.get_alerts(
            is_read=is_read, alert_type=alert_type, device_id=device_id
        )
