...
class CreateShareLinkRequest(BaseModel):
    ...
    permission: SharePermission = Field(
        default=SharePermission.VIEW_ONLY, description="共有権限"
    )
    expires_in_days: int = Field(
        default=7, ge=0, le=365, description="有効期限（日数、0=無期限）"
    )
    shared_with: Optional[List[str]] = Field(
        default=None, description="共有相手リスト（メールアドレスまたはユーザーID）"
    )
...
@router.post(
    "/create-link", response_model=ShareResponse, status_code=status.HTTP_201_CREATED
)
...
async def get_shares_by_recipe(recipe_id: str):
    ...
    return ShareListResponse(
        shares=[_to_share_response(share) for share in shares], total=len(shares)
    )
...
async def send_invite(request: InviteRequest):
    ...
    sharing_service.update_share(share_id=request.share_id, shared_with=new_shared)
...
async def get_share_history(share_id: str):
    ...
    return ShareHistoryResponse(history=history, total=len(history))
