...
class APIKeyService:
  ...
  def check_rate_limit(self, key_id: str) -> Tuple[bool, Optional[str]]:
    ...
    if minute_count >= api_key.rate_limit.requests_per_minute:
      return False, f"Rate limit exceeded: {api_key.rate_limit.requests_per_minute} requests per minute"

    # 時間単位チェック
    hour_ago = now - 3600
    hour_count = sum(1 for r in records if r.timestamp > hour_ago)
    if hour_count >= api_key.rate_limit.requests_per_hour:
      return False, f"Rate limit exceeded: {api_key.rate_limit.requests_per_hour} requests per hour"

    # 日単位チェック
    day_ago = now - 86400
    day_count = sum(1 for r in records if r.timestamp > day_ago)
    if day_count >= api_key.rate_limit.requests_per_day:
      return False, f"Rate limit exceeded: {api_key.rate_limit.requests_per_day} requests per day"
