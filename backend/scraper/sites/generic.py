...
class GenericScraper(BaseScraper):
  ...
  def _extract_servings(self, soup: BeautifulSoup) -> Optional[int]:
    ...
    for text in soup.stripped_strings:
      if any(keyword in text.lower() for keyword in ["servings", "serves", "人分", "人前"]):
        match = re.search(r"(\d+)", text)
        if match:
          return int(match.group(1))
  ...
  def _extract_time(self, soup: BeautifulSoup, time_type: str) -> Optional[int]:
    ...
    keywords = ["prep time", "preparation"] if time_type == "prep" else ["cook time", "cooking"]
    keywords_ja = ["下準備", "準備時間"] if time_type == "prep" else ["調理時間", "加熱時間"]
    ...
    for keyword in keywords + keywords_ja:
      elem = soup.find(string=lambda text: text and keyword.lower() in text.lower())
      if elem:
        parent = elem.parent
        if parent:
  ...
  def _extract_steps(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
    ...
    if not container:
      ...
      for heading in soup.find_all(["h2", "h3", "h4"]):
        heading_text = heading.get_text(strip=True).lower()
        if any(keyword in heading_text for keyword in ["instructions", "directions", "steps", "作り方", "手順"]):
          # Get next sibling or parent's next sibling
