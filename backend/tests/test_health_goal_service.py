...
class TestTDEECalculation:
  ...
  def test_calculate_tdee_moderate_activity(self, service):
    """中活動レベルのTDEE計算テスト"""
    tdee = service.calculate_tdee(30, Gender.MALE, 70.0, 170.0, ActivityLevel.MODERATE)
    bmr = service.calculate_bmr(30, Gender.MALE, 70.0, 170.0)
    expected = bmr * 1.375
    assert abs(tdee - expected) < 1.0
  ...
  def test_calculate_tdee_athlete(self, service):
    """アスリートレベルのTDEE計算テスト"""
    tdee = service.calculate_tdee(30, Gender.MALE, 70.0, 170.0, ActivityLevel.ATHLETE)
    bmr = service.calculate_bmr(30, Gender.MALE, 70.0, 170.0)
    expected = bmr * 1.725
    assert abs(tdee - expected) < 1.0
