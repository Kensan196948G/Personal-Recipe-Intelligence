...
class MealPlanRequest(BaseModel):
    ...
    goals: Optional[List[str]] = Field(
        None, description="目標（weight_loss, muscle_gain等）"
    )
...
async def clear_chat_history(user_id: str = Query(..., description="ユーザーID")):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="ユーザーの履歴が見つかりません"
            )
...
async def analyze_meal(request: MealAnalysisRequest):
    ...
    try:
        ...
        result = advisor_service.analyze_meal(
            user_id=request.user_id, meal_data=meal_data
        )
...
async def get_quick_actions():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"クイックアクション取得エラー: {str(e)}"
        )
